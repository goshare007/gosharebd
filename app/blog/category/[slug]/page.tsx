...
function EmptyState({ categoryName }: { categoryName: string }) {
  return (
    <div className='flex flex-col items-center justify-center py-24 text-center'>
      ...
      <h2 className='text-xl font-bold mb-2'>No posts in this category</h2>
      <p className='text-sm text-muted-foreground max-w-xs mb-6'>
        There are no published posts in &quot;{categoryName}&quot; yet. Check
...
export default function BlogCategoryPage() {
  ...
  const {
    ...
    isPending,
    isError,
  } = usePublicBlogPosts({ category: slug, limit: 12 });
    ...
    <div className='min-h-screen bg-background'>
      ...
      <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12'>
        {isPending ? (
          ...
          <EmptyState categoryName={category?.name || slug} />
        ) : (
          <>
            <div className='mb-6'>
              <p className='text-sm text-muted-foreground'>
                {postsData?.pagination.total} post
                {postsData?.pagination.total === 1 ? '' : 's'} in this category
              </p>
            </div>
            <div className='grid sm:grid-cols-2 lg:grid-cols-3 gap-6'>
